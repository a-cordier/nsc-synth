...
import com.acordier.nsc.view.NscControlledView;
import com.acordier.nsc.view.NscControlledView.ControlFrame;
import com.acordier.nsc.view.NscListBox;
import com.acordier.nsc.view.NscToggle;
...
public class NscViewController {
	...
	public void bindFilter(Knob control) {
		...
		control.setValue(PApplet.map(coreController.getFrequencyValue(),
	...
	public void bindResonance(Knob control) {
		control.addListener(new ControlListener() {
			@Override
			public void controlEvent(ControlEvent event) {
				float value = PApplet.map(event.getValue(), 0, 127, RES_MIN,
						RES_MAX);
	...
	public void bindGain(Knob control) {
		control.addListener(new ControlListener() {
			@Override
			public void controlEvent(ControlEvent event) {
				float value = PApplet.map(event.getValue(), 0, 127, GAIN_MIN,
						GAIN_MAX);
		...
		control.setValue(PApplet.map(coreController.getGainValue(), GAIN_MIN,
				GAIN_MAX, 0, 127));
	}
	...
	public void bindAdsr(Group controls) {
		controls.getController("Amp.").addListener(new ControlListener() {
			...
			@Override
			public void controlEvent(ControlEvent event) {
				float value = PApplet.map(event.getValue(), 0, 127, AMP_MIN,
						AMP_MAX);
		...
		controls.getController("Amp.").setValue(
				PApplet.map(coreController.getAmpValue(), AMP_MIN, AMP_MAX, 0,
						127));
		...
		controls.getController("Att.").setValue(
				PApplet.map(coreController.getAttackValue(), ATT_MIN, ATT_MAX,
						0, 127));
		...
		controls.getController("Dec.").setValue(
				PApplet.map(coreController.getDecayValue(), DEC_MIN, DEC_MAX,
						0, 127));
		...
		controls.getController("Sus.").setValue(
				PApplet.map(coreController.getDecayValue(), SUS_MIN, SUS_MAX,
						0, 127));
		...
		controls.getController("Rel.").setValue(
				PApplet.map(coreController.getDecayValue(), REL_MIN, REL_MAX,
						0, 127));
	}
	...
	public void bindVcoGain(Knob control) {
		if (control.getName().matches("^.*1$")) {
			control.addListener(new ControlListener() {
				...
				public void controlEvent(ControlEvent event) {
					float value = PApplet.map(event.getValue(), 0, 127,
							VCO_GAIN_MIN, VCO_GAIN_MAX);
					coreController.setVco1GainValue(value);
					NscLogger.logEvent(event, value);
				}
			});
			control.setValue(PApplet.map(coreController.getVco1GainValue(),
					VCO_GAIN_MIN, VCO_GAIN_MAX, 0, 127));
		} else {
			control.addListener(new ControlListener() {
				@Override
				public void controlEvent(ControlEvent event) {
			...
			control.setValue(PApplet.map(coreController.getVco2GainValue(),
					VCO_GAIN_MIN, VCO_GAIN_MAX, 0, 127));
		}
	}
	...
	public void bindVcoOctave(Knob control) {
		if (control.getName().matches("^.*1$")) {
			control.addListener(new ControlListener() {
				@Override
				public void controlEvent(ControlEvent event) {
					int value = PApplet.ceil(PApplet.map(event.getValue(), 0,
							4, VCO_OCT_MIN, VCO_OCT_MAX));
					coreController.setVco1OctaveValue(value);
					NscLogger.logEvent(event, value);
				}
			});
			control.setValue(PApplet.map(coreController.getVco1OctValue(),
					VCO_OCT_MIN, VCO_OCT_MAX, 0, 4));
		} else {
			control.addListener(new ControlListener() {
				@Override
				public void controlEvent(ControlEvent event) {
			...
			control.setValue(PApplet.map(coreController.getVco2OctValue(),
					VCO_OCT_MIN, VCO_OCT_MAX, 0, 4));
		}
	}
	...
	public void bindDelayTime(Knob control) {
		control.addListener(new ControlListener() {
			...
			public void controlEvent(ControlEvent event) {
				float value = PApplet.map(event.getValue(), 0, 127,
						DEL_TIME_MIN, DEL_TIME_MAX);
				coreController.setDelayTimeValue(value);
				NscLogger.logEvent(event, value);
			}
		});
		control.setValue(PApplet.map(coreController.getDelayTimeValue(),
				DEL_TIME_MIN, DEL_TIME_MAX, 0, 127));
	}
	...
	public void bindDelayFeedBack(Knob control) {
		control.addListener(new ControlListener() {
			@Override
			public void controlEvent(ControlEvent event) {
				float value = PApplet.map(event.getValue(), 0, 127, DEL_FB_MIN,
						DEL_FB_MAX);
				coreController.setDelayFeedBackValue(value);
				NscLogger.logEvent(event, value);
			}
		});
		control.setValue(PApplet.map(coreController.getDelayFeebackValue(),
				DEL_FB_MIN, DEL_FB_MAX, 0, 127));
	}
	...
	public void bindBitCrushResolution(Knob control) {
		control.addListener(new ControlListener() {
			@Override
			public void controlEvent(ControlEvent event) {
				float value = PApplet.map(event.getValue(), 0, 127, BC_MIN_RES,
						BC_MAX_RES);
				coreController.setBitCrushResolutionValue(value);
				NscLogger.logEvent(event, value);
			}
		});
		control.setValue(PApplet.map(
				coreController.getBitCrushResolutionValue(), BC_MIN_RES,
				BC_MAX_RES, 0, 127));
	}
	...
	public void bindBitCrushRate(Knob control) {
		control.addListener(new ControlListener() {
			...
			public void controlEvent(ControlEvent event) {
				float value = PApplet.map(event.getValue(), 0, 127,
						BC_MIN_RATE, BC_MAX_RATE);
				coreController.setBitCrushRateValue(value);
				;
				NscLogger.logEvent(event, value);
			}
		});
		control.setValue(PApplet.map(coreController.getBitCrushRateValue(),
				BC_MIN_RATE, BC_MAX_RATE, 0, 127));
	}
//
...
//	}
	...
	public void bindMidiInputDeviceSelector(NscListBox control) {
		List<MidiDevice> devices = coreController.getMidiInputDevices();
		int i = 0;
		for (MidiDevice device : devices) {
			control.addItem(device.getDeviceInfo().getName(), i);
		...
		final NscListBox _control = control;
		control.addListener(new ControlListener() {
			@Override
			public void controlEvent(ControlEvent event) {
				int idx = ((int) event.getValue());
